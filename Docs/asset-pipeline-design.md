...
- **Vulkan backend**: in-flight frames and cached descriptor sets mean an in-place GL-style swap isn't enough; route through `WaitIdle` + `InvalidateSceneCaches` (the machinery material edits already use). GL first.
